...
class CellSpace(CoupledDEVS):
    def __init__(self, x_max, y_max, burn_x, burn_y, temperature, wind_dir):
        ...
        for x in range(x_max):
            ...
            for y in range(y_max):
                if x == burn_x and y == burn_y:
                    row.append(self.addSubModel(BurningCell(x, y, wind_dir)))
                else:
                    row.append(self.addSubModel(Cell(x, y, temperature, wind_dir)))
            cells.append(row)
